"""Deterministic preparation-time-spike detector.

Responsibility
--------------
Given the mean preparation time (seconds) in the current window versus the
historical baseline, decide whether kitchen throughput is degraded.

Trigger conditions (both must be met):
  1. completed_count >= 8             (minimum sample guard)
  2. current_mean >= baseline_mean × 1.40  (ratio threshold)
...
MIN_COMPLETED_COUNT = 8
RATIO_THRESHOLD = 1.40
DETECTOR_VERSION = "prep_time_spike.v1"
...
class PrepTimeMetrics:
    """Input value object for the preparation-time detector.

    Attributes
    ----------
    restaurant_id       : Restaurant identifier.
    current_mean_seconds: Mean prep time in the observation window.
    baseline_mean_seconds: Historical mean prep time (same time band).
    completed_count     : Completed preparations in the window.
    window_start        : UTC-aware window start.
    window_end          : UTC-aware window end.
    evidence_ids        : Non-empty unique event IDs for completed preps.
    """
...
def detect_prep_time_spike(metrics: PrepTimeMetrics) -> Signal | None:
    ...
    if metrics.completed_count < MIN_COMPLETED_COUNT:
        return None
    ...
    signal_id = f"sig_prep_time_{metrics.outlet_id}_{window_tag}"
