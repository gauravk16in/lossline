...
async def post_event(client: httpx.AsyncClient, api_url: str, event: dict) -> bool:
    ...
    try:
        ...
        if response.status_code in [200, 202]:
            data = response.json()
            logger.debug(f"Event {event['event_id']} accepted. Duplicate: {data.get('duplicate')}")
            return True
        else:
            logger.error(f"Failed to post event {event['event_id']}: Code {response.status_code} - {response.text}")
            return False
...
async def wait_for_manager_approval(api_url: str) -> None:
    ...
    """
    logger.info("Simulation paused at degradation phase. Waiting for manager approval...")
    
    ws_url = api_url.replace("http://", "ws://").replace("https://", "wss://") + "/api/v1/ws"
    ...
    async def listen_ws():
        try:
            ...
            logger.warning(f"WebSocket connection failed or disconnected: {e}. Falling back to polling.")
                    ...
                    if response.status_code == 200:
                        ...
                        for inc in incidents:
                            ...
                            if inc.get("status") == "ACTION_APPROVED":
                                logger.info("Manager approval detected via REST polling!")
                                approval_event.set()
                                return
                except Exception as e:
    ...
    async with httpx.AsyncClient() as client:
        ...
        try:
            ...
            if reset_resp.status_code != 200:
                logger.error(f"Reset database failed: Code {reset_resp.status_code} - {reset_resp.text}")
                sys.exit(1)
        ...
        logger.info(f"Fast-posting {len(baseline_events)} historical baseline events in parallel...")
        ...
        logger.info(f"Baseline loaded successfully ({success_count}/{len(baseline_events)} events ingested).")

        # 4. Stream pre-approval live events (Healthy -> Surge -> Degradation)
        logger.info(f"Streaming {len(pre_approval_events)} live events with speed multiplier {speed}...")
...
def main():
    parser = argparse.ArgumentParser(description="LOSSLine operational scenario simulator.")
    parser.add_argument(
        "--api-url",
