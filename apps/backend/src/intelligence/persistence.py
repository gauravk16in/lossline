...
def _as_utc(value: datetime) -> datetime:
    """Normalize timestamps returned by SQLite and PostgreSQL alike."""
...
async def _link_signals(
    ...
    for row in signal_rows:
        if row.id in have:
            continue
        await db.execute(
            incident_signals.insert().values(incident_id=incident_id, signal_id=row.id)
        )
...
async def persist_incident_from_candidate(
    ...
    *,
    ...
    explanation_provider_model: str | None = None,
) -> Incident:
    ...
    components: dict[str, Any] = {
        ...
        "explanation_provider_model": explanation_provider_model,
    }
    explanation_text = _render_explanation(explanation)
...
def _render_explanation(explanation: dict[str, str] | None) -> str:
    if not explanation:
        return (
            "Operating metrics changed from baseline. The correlated evidence "
            "suggests operational overload, not proof of causation."
        )
    return " ".join(
        explanation[field]
        for field in (
...
async def _upsert_recommendation(
    ...
    expires_at = datetime.now(timezone.utc) + timedelta(
        minutes=settings.RECOMMENDATION_EXPIRY_MINUTES
    )
