...
def _order_cancelled(i: int, occurred_at: str = "2026-08-08T12:15:00Z") -> dict[str, Any]:
    return {
        "schema_version": "1.0",
...
async def test_pipeline_persists_cancellation_signal_and_incident(
    ...
    assert signals[0].baseline_value == pytest.approx(0.07)

    incidents = (
        await db_session.execute(select(Incident))
    ).scalars().all()
