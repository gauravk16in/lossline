...
def _avg_order_value(snapshot: MetricSnapshot, events_amounts: list[Decimal]) -> Decimal | None:
...
async def run_detection_pipeline(envelope: EventEnvelope) -> None:
    ...
    async with SessionLocal() as db:
        ...
        if candidate is not None:
            ...
            amounts = [
                e.amount for e in current_events if e.amount is not None
            ]
            aov = _avg_order_value(snapshot, amounts)
            window_minutes = max(
                1.0, (snapshot.window_end - snapshot.window_start).total_seconds() / 60.0
            )
            ...
            cancel_sig = next(
                (s for s in domain_signals if s.signal_type.value == "CANCELLATION_SPIKE"),
                None,
            )
            revenue = estimate_revenue_at_risk(
                RevenueInputs(
                    observed_cancelled_value=observed,
                    current_cancel_rate=cancel_sig.current_value if cancel_sig else snapshot.cancellation_rate,
            ...
            if confidence.tier == ConfidenceTier.MONITOR_ONLY or domain_rec is None:
                status = "MONITOR_ONLY"
            else:
            ...
            # NOTE: LangGraph investigation starts here in Phase 3 (deferred).
            return
